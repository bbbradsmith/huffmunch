...
; out: reads 1 byte from standard huffmunch (non-RLE) stream, result in A (X,Y,flags clobbered)
.export huffmunch_read

; out: reads 1 byte from huffmunch RLE stream, result in A (X,Y,flags clobbered)
.export huffmunch_read_rle
...
hm_rle_len = <(huffmunch_zpblock + 9) ; bytes left in RLE packet
hm_rle_val = <(huffmunch_zpblock + 10) ; repeated byte in RLE packet
.assert (huffmunch_zpblock + 11) <= 256, error, "huffmunch_zpblock requires 11 bytes on zero page"
...
packet_start:
	...
	sta hm_rle_len
	cmp #$80
	bcc huffmunch_read_rle ; raw packet (A bytes)
	jsr huffmunch_read
	sta hm_rle_val
	jmp huffmunch_read_rle ; run packet (256-A bytes)
.endproc
